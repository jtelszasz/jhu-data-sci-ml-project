...
User names and a variable for indicating new time windows are included in the dataset.

```{r}
levels(all_training_data$user_name)
levels(all_training_data$new_window)
```
...
training <- all_training_data[,na_pct < 0.9]
table(na_pct<0.9)
```

60 fields remain that have less than 90% NA values.
...
A boosted tree model is trained on the training set using the caret package (train function with "gbm" method).  K-folds cross-validation is performed using the default 10 folds.  The most accurate model uses 150 trees with an interaction depth of 3.

```{r model, cache=TRUE, results=FALSE}
start.time <- Sys.time()
...
Out of sample accuracy is calculated using the validation set of 9,812 samples not used for training the model and is estimated to be 97.9%.  Sensitivity is estimated to be 98.9% and specificity is approximately 99.8%.  
